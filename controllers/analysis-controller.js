...
const getFullMedicalRecord = (patient, medical_record) => {
    const now = new Date();
    const formattedNow = now.getFullYear().toString() + now.getMonth().toString()
    return {
        "id": patient.id || '',
        "state": patient.state || '',
        "healthcare_unit": medical_record.healthcare_unit || '',
        "issue_date": now.getFullYear().toString() + now.getMonth().toString() || '',
        ...
        "reason_for_discharge": 0, //medical_record.reason_for_discharge || '',
        "date_of_discharge": medical_record.reason_for_discharge ? formattedNow : '',
        ...
        "indicator_of_transplantation": medical_record.indicator_of_transplantation || '',
        "number_of_transplantation": medical_record.number_of_transplantation || '',
    }
}
